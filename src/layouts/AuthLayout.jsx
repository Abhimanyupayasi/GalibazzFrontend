...
// 2nd approach better localhost dont know about production
...
//   useEffect(() => {
//     let timeout; // For setting timeout to redirect if verification takes too long
...
//         timeout = setTimeout(() => {
//           setLoading(false);
//           navigate('/'); // Redirect if verification takes more than 3 seconds
//         }, 3000);

//         const isValid = await handleVerifyToken(token);
        
//         clearTimeout(timeout); // Clear timeout if verification completes before 3 seconds
...
function AuthLayout() {
  const [showNotification, setShowNotification] = useState(true);
  const [notificationMessage, setNotificationMessage] = useState('Successfully Logged In');
  ...
  // No need to verify the token, just check if it's present
  useEffect(() => {
    if (!token) {
      setNotificationMessage('Pls Login Again.. Redirecting...');
      setShowNotification(true);
      setTimeout(() => {
        dispatch(logout());
        localStorage.clear();
        navigate('/');
      }, 2000); // Redirect after 2 seconds if token is not present
    }
  }, [token, dispatch, navigate]);
  ...
  return (
    <>
      ...
      {token ? (
        ...
        <div className="flex flex-col justify-center items-center h-screen">
          <div className="mb-4 text-center">
            <p className="text-red-500">No token found. Please log in again.</p>
          </div>
          <LoginButton />
        </div>
      )}
